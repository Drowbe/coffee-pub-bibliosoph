...
- **The camera account needs no setup to see the crit.** Bibliosoph now declares its four channels to Blacksmith at startup (`api.toast.registerChannel`), so they render as labelled, tickable rows — Critical Hits, Fumbles, Injuries, Table Breaks — instead of magic words a GM had to know to type. This came out of a real lost session: the first version worked exactly as designed and delivered nothing, because an empty allow-list allowed nothing and no part of the UI admitted the words `crit` or `fumble` existed. Blacksmith also flipped the default, so an empty list now permits every *declared* channel: declaring one is already the sender saying "this isn't routine chatter", and the setting is how a GM narrows that rather than how they switch it on. Guarded with `typeof registerChannel === 'function'`, so older builds simply skip it.
