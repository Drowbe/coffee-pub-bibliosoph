...
- **Criticals, fumbles and inspiration cards import through Blacksmith's JSON importer, the same way injuries do.** Three new registrations over two models: `scripts/data/outcome-import-profile.js` registers `critical` and `fumble` against `OutcomePageModel`, and `scripts/data/inspiration-import-profile.js` registers `inspiration` against `InspirationPageModel`. Like injuries, neither writes a machine shape: field names, types, bounds, enums, defaults and nullability are derived from the model by Blacksmith's `declarationFromModel`, and the files supply only the human layer, the envelope fields and the document block. All four profiles now register from one `ready` hook in `bibliosoph.js`, each reporting its own failure rather than aborting the rest, so a rejected outcome declaration does not silently cost the GM injury import as well.

  **Two outcome profiles rather than one, with `kind` declared as a `const`.** Criticals and fumbles share `coffee-pub-bibliosoph.outcome`, so one profile with an authored `kind` was the obvious shape and is the wrong one. A const cannot be authored or mistyped, where an authored field lets a payload claim to be a crit while carrying fumble semantics with nothing to catch it. It also halves the container problem, because with `kind` fixed per profile the journal name depends on `severity` alone. And guidance is one sentence by contract: a single profile would have to name all six bucket labels in it, where two name three each in the author's own vocabulary. The const is WITHHELD from the walk rather than layered over it: Blacksmith concatenates `extraFields` onto the derived fields and rejects a duplicate name at registration, so declaring the const while the walk still emits the model's `kind` fails with "duplicate field name". The verifier caught that before it reached a world.

  **The container names are not stored anywhere.** `system.severity` is minor/moderate/major while the journals are Butchery, Carnage, Slaughter and Meek, Nasty, Devastating, and that mapping is not a casing of the stored value: `minor` is Butchery for a crit and Meek for a fumble. It reaches Blacksmith as `containerNameMap`, derived from `SEVERITY_LABELS` rather than transcribed, which keeps it module-owned data carried by a Blacksmith-owned mechanism. Inspiration takes the other new form, `containerName`, because the whole deck lives in one journal and the model has no category-like field to point `containerNameFrom` at, which is what made that profile unwritable before.

  **Each profile asks the author what narrows its generation.** Outcomes ask for a bucket, which also decides the journal the pages are filed into, and a count; inspiration asks for a count and an optional automatable action. Options derive from the schema constants, so a value added to `SEVERITIES` or `ACTION_KEYS` appears without anyone remembering.

- **Every profile resolves its image path instead of trusting it.** A generated payload picks icon paths from the model's memory and roughly one in ten does not exist, which imports cleanly, validates, and renders a broken card: valid JSON, valid schema, wrong world. All four profiles now declare `transform: 'resolveImage'` with the narrowest `imageRoots` covering what they already ship and a mandatory `imageFallback`. A supplied path that exists is returned untouched after browsing its own directory alone; one that does not is matched against the roots on shared filename words; a filename that matches exactly short-circuits regardless of folder; and anything with nothing close lands the declared default with a warning naming the field.

  **The roots are wider than they look, and that is the corpus rather than carelessness.** Injuries are filed by DAMAGE TYPE, so acid, sonic, air and lightning wounds have no art under `icons/skills/wounds` and ten of the fourteen categories draw from `icons/magic`: four roots and about 3,000 files. Cost is not the constraint it appears to be, because roots are walked once per session and only on a miss.

  **Measured on our own 248 shipped records rather than asserted.** Taking art a person actually chose, corrupting it the way a generator misremembers, and asking whether the resolver returns the original: 97% when a filename word is wrong, 100% when the filename is right and the folder is wrong, no dead paths in either. The measurement found four defects in Blacksmith's resolver that its own harness could not, because its corpus was one root of core icons against a synthetic corruption and ours is four roots of art chosen by hand. Two of those were found by reading a log line that no pass or fail reflected.

  **The headline rate hid a regression once, which is worth recording.** An intermediate build moved exact recovery from 78% to 89% while wrong-in-a-DIFFERENT-FAMILY went from 2 to 7, turning honest fallbacks into confident wrong answers: a bell for blood cells, a pine cone for a glowing heart. A rate cannot express the difference between a visible non-answer and a silent wrong one, and for a GM at a table that difference is the whole product question. Wrong-in-a-different-family is the number this feature is judged on.

  **`image` is withheld from the schema walk and declared by hand**, like `kind` on the outcome profiles, because `extraFields` are concatenated onto the derived fields rather than merged and the walk cannot carry `transform`, `imageRoots` or `imageFallback`. Its guidance names the SHAPE of a path and the directories to draw from rather than a complete example: a concrete value in a one-sentence guidance field is read by a generator as the answer instead of the format, which produced two quite different fumbles illustrated with the same dagger.

- **A build gate for the three new declarations.** `tools/verify-content-profiles.mjs`, available as `npm run content:profiles` and run by `content:build`. Injuries keep their own verifier because it also closes the container loop against the shipped compendium, which needs injury knowledge; these three have simpler containers and share one caller. It builds each declaration offline, puts it through Blacksmith's own `validateDeclaration` rather than a local reading of its rules, and checks that every model field is declared and every declared field has a counterpart, that every field carries exactly one sentence of guidance, that the document builds a `JournalEntryPage` of the right subtype, that there is exactly one selector listing the profile's own id, and that a `foldername` input exists so the import can land somewhere other than the root. It also checks what only this caller can know: that each outcome map covers every severity with the name the compendium actually ships, and that inspiration's constant container is the deck's journal name. The Foundry stubs carry class defaults for the reason the injury verifier records: a thin stub does not make the check weaker in an obvious way, it makes it check a different object.

### Changed

- **The token-versus-actor rule is written into `architecture-bibliosoph.md`.** Resolve by token to identify or apply; resolve by actor to aggregate. A copied token does not get its own actor, so `actor.id` on an unlinked token is the id the whole group shares, and writing to it hits every copy while reading from it describes the prototype. The section records both halves, including that it presents as damage or effects "propagating between copies" and is not: isolation is intact and the display is lying. Written down because the rule is obvious once stated and was not stated, so the same mistake was made independently in six places here and two in Blacksmith, each looking like a local detail rather than an instance of a class.
