...
- **Walk the nine user guides in a running world.** All were written from source and `lang/en.json`, not from a live session. Source says what exists; it does not say what order things appear in or what a control is called on screen, and those are the two things a reader navigates by. What is unverified, per guide:
  - `userguide-getting-started.md` -- the toolbar button order in the table, and the button labels themselves, which come from code rather than from `lang/en.json`.
  - `userguide-messages.md` -- where the favourite star sits in each window, whether the tone bar and reaction chips are where the guide says, and the wording of the tray rows.
  - `userguide-injuries.md` -- the four picker controls and their icons, and the Check-Up card's row layout and grouping.
  - `userguide-authoring-injuries.md` -- the injury sheet's field order, and whether the page type is chosen the way the guide describes.
  - `userguide-outcomes.md` -- the mechanics line's exact rendering, and which apply buttons appear for each targeting mode.
  - `userguide-inspiration.md` -- the GM deal dialog's contents, and what the play card looks like for each action.
  - `userguide-investigation.md` -- the card's zones and what a failed find actually shows.
  - `userguide-encounters.md` -- every control in the Quick Encounter window; this one was written almost entirely from the architecture document and is the least verified of the nine.
