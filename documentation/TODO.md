...
- **Reshoot the two captures that show real people.** `bibliosoph-product.webp` and `bibliosoph-communications.webp` both publish real players' full names to the GitHub landing page and the wiki. They are in the conversation tray rows, the group-conversation titles, and the "X wrote:" quoted reply block -- interface chrome rather than anything anyone would think of as content, which is why it is easy to miss. Recapture in a scratch world with fictional users. Verify: read every string in the frame, not just the prose.
