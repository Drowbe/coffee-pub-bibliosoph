...
- **Blacksmith owes us declaration-level `promptFields` (phase 2, charter).** A registered profile can contribute a label and one sentence of guidance per field and nothing else. Blacksmith's own three journal profiles get Realm/Region/Site, a folder field, image pickers and generation dropdowns from kind-level getters (`registry-json-import-journals.js:1438-1444`) that no declaration can reach, while `promptFields` is read from the kind and nowhere else (`registry-json-import.js:390`). That is Blacksmith enhancing its own declared profiles through a side channel, which the consumer-zero charter forbids. For injuries the three that would change output quality rather than convenience are `category` (it decides which journal a generated injury lands in, so without it a generation scatters and the GM re-files by hand), `severity` (every numeric band we publish is severity-scoped, so fixing it collapses conditional guidance into fixed numbers) and a count. Not injury-specific: outcomes have the identical severity-band structure and inspiration wants a count, which is the test of whether it generalises. Filed with Blacksmith and deferred by them deliberately, on the grounds that an authoring surface on top of an import path that had never landed a page would be building on static verification -- that objection is now spent. Verify: a registered profile can ask the author for a value and the answer constrains the generated payload.
