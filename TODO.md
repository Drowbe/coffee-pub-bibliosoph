...
- **Send `documentation/note-blacksmith-effects-and-tooling.md`.** Written 2026-08-07, revised same day, awaiting send. Two items already landed in Blacksmith (duration formatting in `getDisplayEffects`; our authoritative classifier, which changes what their combat bar renders). Four need their decision, of which **§B is the real one**: the effects layer should adapt the effects ecosystem the way `utility-midi-resolution.js` already adapts Midi-QOL — detect, yield or supply the baseline, expose one contract — so no satellite ever branches on whether Times Up is installed. Duration normalization and expiry detection come with it, since Blacksmith already owns effect display and the combat timers. §A notes that `architecture-effects.md` lists "Midi-QOL or DAE interpretation", "socket synchronization" and "duration ticking or expiry" as non-goals while Blacksmith does all three elsewhere. The other three are small: a stale `architecture-effects.md`, two `wiki-sync.mjs` backports (Windows `EPERM`, cross-module wiki links), and whether the classifier/duration boundary is intended. **Make sure the revised copy goes out** — the first draft asked what sets `duration.rounds` (answered, but as a Times Up dependency rather than an ownership gap) and claimed world time can be frozen (wrong; it advances 6s/round, and injury durations are simply on a downtime scale).
...
- **Our expiry must hold without Times Up, and survive it.** Times Up — optional, so never a dependency — rewrites any effect with `duration.seconds <= MaxRoundsToConvert × roundTime` (default 60s) into rounds and nulls `duration.seconds`. Where it is installed: `expiry: linger` is silently overridden when its **Expire rounds/turns duration effects on combat end** setting deletes the converted effect, and its own expiry competes with `manager-injury-ticks` — the likely reason an expiring critical produced a burst but no Bibliosoph announcement. Affects 60s injuries and every crit/fumble at 10 rounds or fewer. **Do not read `flags.times-up.durationSeconds`** — building on an optional module's internals is the thing we do not do. `hasExpired()` reads `duration.remaining`, which Foundry keeps correct for both unit types, so the standalone path is already sound; what needs verifying is that our expiry still wins when Times Up is present, and what to do when it does not. The durable fix is the duration/expiry service asked for in `note-blacksmith-effects-and-tooling.md` §B — once Blacksmith emits an expiry event, we stop detecting the clock ourselves and only decide policy.
