...
- **Player treatment via Medicine checks (committed, 2026-07-27).** Let players treat afflictions themselves: the injury card / Check-Up row carries a roll target (DC — likely severity-derived), the player rolls Medicine through Blacksmith's roll tools, success removes the affliction / failure leaves it (maybe with a per-rest retry limit). Open design: who may roll (self vs. anyone adjacent), interaction with the GM's icon-button Treat.
- **Treatment inventory requirements (committed, 2026-07-27).** Gate player treatment on supplies — Healer's Kit / bandages / medkit in inventory, consuming a use per treatment (or granting the roll advantage vs. bare-handed). Needs an item-matching strategy (name match vs. a configurable item list).
