...
5. ~~**Request-side advantage/disadvantage for `openRequestRollDialog`.**~~ **DELIVERED** (Blacksmith master, `## [Unreleased]` — not yet tagged). Adopted 2026-07-30: treatment requests now pass `rollAdvantage` (request-level and per-actor), `lockRollAdvantage: true`, and `explanation`. `'normal'` is a real requestable value, which is exactly what the "kit and self cancel out" row needed. The formula-sniffing survives as a cheap **assertion** at Blacksmith's own suggestion — enforcement lives at the buttons, not in the roll path, so a mismatch on a *locked* request would be a real bug in their layer and this is the only place that would notice. Reads the mode off `flags['coffee-pub-blacksmith']` now rather than the dice formula. Their one gap: the GM's own Request a Roll dialog has no UI for either field (requester-side only), which does not block us. **Safe to ship before their release** — an older Blacksmith ignores the unknown options and falls back to the previous behaviour. Original request text follows.

   **Request-side advantage/disadvantage for `openRequestRollDialog`.** The silent request API carries DC, `situationalBonus`, and `customModifier`, but advantage/disadvantage can only be chosen by the *roller's* buttons at roll time — a requesting module cannot force or default the mode. Ask for per-actor and global `advantage`/`disadvantage` options on the request (honored in the roll window and cinematic mode, ideally locking the buttons or at least pre-selecting), plus a **custom explainer/description field** rendered on the request card (`showRollExplanation` only toggles the standard skill description — a requester can't say "your Healer's Kit grants Advantage here"; today that guidance has to ride the title or module-side toasts/tooltips). Until then Bibliosoph's treatment rolls state the required mode in the request title and the GM resolver detects the actual mode from the roll formula (2d20kh/kl), logging mismatches.
