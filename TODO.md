...
- **Player self-treatment (idea, 2026-07-27).** Let the afflicted player heal themselves via a Medicine check: the injury card (and/or Check-Up row) carries a roll target (DC), the player clicks to roll Medicine through Blacksmith's roll tools, and success removes the affliction / failure leaves it (maybe with a cooldown). Possibly gate on inventory — requires a Healer's Kit / bandages, consuming a use on treatment. Needs design: where the DC comes from (severity?), who may roll (self vs. anyone adjacent), and how it interacts with the GM's icon-button Treat.
